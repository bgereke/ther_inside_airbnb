...
missing_prof <- profile_missing(detailed_listings)

options(repr.plot.height = 600)
ggplot(data = missing_prof[missing_prof$num_missing > 0,], aes(x = feature, y = pct_missing)) + 
  geom_bar(stat = "identity") +
  xlab('feature') + ylab('proportion missing') +
  ggtitle('features with missing data') +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) 
```
