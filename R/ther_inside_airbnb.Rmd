...
calendars <- full_df %>%
  ...
  # mutate(price = price / max(price) - 0.5) %>%
  ungroup %>%
...
calendars <- calendars[,2:366]
calendars <- as.data.frame(t(as.matrix(calendars)))

specs <- map(calendars, ts) %>%
  map(spec.mtm, plot = FALSE, nw = 10) %>%
  map(purrr::pluck, "spec")

specs <- t(matrix(replace_na(unlist(specs),0), ncol = ncol(calendars)))
# specs <- apply(t(specs), 2, function(x) x - min(x))
# specs <- t(apply(specs, 2, function(x) x / max(x)))
specs <- apply(specs, 2, function(x) x / sum(x))
  

umapped <- umap(specs)
comps <- prcomp(specs, scale = TRUE, center = TRUE)
clusts <- dbscan(comps$x[,1:3], eps = 2)

tsne <- Rtsne(specs, check_duplicates = FALSE)

clust <- tsclust(specs, 
                 type = "tadpole",
                 k = 7,
                 control = tadpole_control(dc = 1.5, window.size = 10))

clust <- tsclust(specs, k = 6, distance = "L2", centroid = "pam")
...
rgl.points(comps$x[,1], comps$x[,2], comps$x[,3], color = 'black', size = 5)
```
