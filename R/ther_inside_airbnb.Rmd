...
ggplot(data = sample_df, aes(x = date, y = price, group = listing_id, color = listing_id)) + 
  geom_line(size = 1) +
  xlab('date') + ylab('price (normalized)')
```
From these visualizations, we can tell that price dynamics vary substantially from listing to listing and many appear non-stationary. Predictions will be better if we can identify groups of similarly behaving listings and model them separately. Let's reduce dimensionality with umap and cluster the resulting embedding with dbscan: 
```{r}
library(dtwclust)
...
clust <- tsclust(as.matrix(calendars[,-1]), 
                 type = "tadpole",
                 k = 10,
                 control = tadpole_control(dc = 1.5, window.size = 18))

data_plot <- data.table(melt(umapped))
data_plot[, Clust := rep(res_clust$cluster, ncol(umapped))]
...
dbclust <- as.matrix(umapped$x[,1:3]) %>%
  dbscan(eps = 1) 
umap_df <- data.frame('umap1' = umapped$x[,1],
                      'umap2' = umapped$x[,2],
                      'clust' = factor(dbclust$cluster))
ggplot(data = umap_df, aes(x = umap1, y = umap2, group = clust, color = clust)) + 
  geom_point()

rgl.open()# Open a new RGL device
rgl.bg(color = "white") # Setup the background color
rgl.points(umapped$x[,1], umapped$x[,2], umapped$x[,3], color = get_colors(dbclust$cluster), size = 5)
```
This looks pretty decent. Let's plot the time series from the different groups to verify that they appear similar.
```{r}
...
# xgtrain <- xgb.DMatrix(data = makeX(full_df[!(as.integer(full_df$listing_id) %in% test_listings),][,features], 
#                                     sparse = TRUE),
#                       label = full_df$log_price[!(as.integer(full_df$listing_id) %in% test_listings)])
# 
# xgtest <- xgb.DMatrix(data = makeX(full_df[as.integer(full_df$listing_id) %in% test_listings,][,features], 
#                                    sparse = TRUE),
#                       label = full_df$log_price[as.integer(full_df$listing_id) %in% test_listings])
...
# xgmod <- xgboost(data = xgtrain_data, 
#                  label = xgtrain_lab,
#                  params = params,
#                  nrounds = xgbcv$best_iteration)
# 
# xgpred <- predict(xgmod, xgtest_data)
r_squared <- 1 - sum((xgtest_lab - xgpred)^2) / sum((xgtest_lab - mean(xgtest_lab))^2)

importanceRaw <- xgb.importance(feature_names = colnames(xgtrain_data), model = xgmod)
